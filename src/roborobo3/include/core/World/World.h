...
class World
{
    ...
    public:
		...
		int getIterations();
		int getGenomeReservoirGlobalSize() { return (int)_genomesListGlobal.size(); }
		int getSigmaReservoirGlobalSize() { return (int)_sigmaListGlobal.size(); }

		std::map< std::tuple<int,int,int>, std::vector<double> > _genomesListGlobal;
		std::map< std::tuple<int,int,int>, float > _sigmaListGlobal;
		//std::map< std::pair<int,int>, std::vector<double> > getGenomeReservoir() {return _genomesListGlobal;}
