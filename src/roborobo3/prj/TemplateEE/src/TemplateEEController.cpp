...
//bool TemplateEEController::receiveGenome(std::vector<double> __genome, std::pair<int,int> __senderId, float __sigma, float __fitness) // sigma and fitness are optional (default: 0)
{
    ...
    else
    {
        ...
        gWorld->_genomesListGlobal[std::make_tuple(p->senderId.first, p->senderId.second, _wm->getId())]=p->genome;
        gWorld->_sigmaListGlobal[std::make_tuple(p->senderId.first, p->senderId.second, _wm->getId())]=p->sigma;
