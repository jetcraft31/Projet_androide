...
bool loadProperties( std::string __propertiesFilename )
{
    ...
    if ( gProperties.hasProperty("gNbOfArena") )
    {
        convertFromString<int>(gNbOfArena, gProperties.getProperty("gNbOfArena"), std::dec);
    }
    else
    {
        std::cerr << "[MISSING] gNbOfArena string value is missing. The missing value will be 1\n";
    }
