...
void Robot::move( int __recursiveIt ) // the interface btw agent and world -- in more complex envt, this should be handled by the "world".
{
	...
	//ON garde en mémoire les anciennes valeurs pour le calcul

	if(gWorld->getIterations()%5 ==0){
		_wm->_xRealLast = _wm->_xReal;
		_wm->_yRealLast = _wm->_yReal;
	}

	// 250 car j'ai pris comme le prof, c'est à dire 4 pixel, du coup 1000/4 = 250 cases différentes	
	if( ( (int)xReal_old%250 != (int)_wm->_xReal%250) || ( (int)yReal_old%250 != (int)_wm->_yReal%250) ){
		_wm->_caseMap[(int)_wm->_xReal%250][(int)_wm->_yReal%250] ++;
	}
